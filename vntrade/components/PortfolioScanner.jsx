...
function VPABar({ score }) {
  ...
  const col = score >= 13 ? "#00d97e" : score <= 7 ? "#ff4560" : "#f0c040";
  return (
...
function RRBadge({ rr }) {
  const col = rr >= 2 ? "#00d97e" : rr >= 1.5 ? "#f0c040" : "#ff9040";
  return (
...
function StockCard({ res, color, onSelect }) {
  ...
  return (
    <div
      style={{
        background: "#05101a",
        border: `1px solid ${open ? color + "50" : "#0d1f2e"}`,
    ...
    >
      ...
      <div style={{
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 10, minWidth: 0 }}>
          <div>
            <div style={{ display: "flex", alignItems: "center", gap: 7 }}>
              ...
              {res.goldenX && <Tag color="#f0c040">✦ Golden X</Tag>}
              {res.aboveE200 && <Tag color="#3b9eff">▲ E200</Tag>}
              {res.vsaBullish.slice(0,1).map(s => <Tag key={s} color="#00d97e">{s}</Tag>)}
            </div>
            <div style={{ fontSize: 10, color: "#2a5060", marginTop: 1 }}>
        ...
        <div style={{ textAlign: "center" }}>
          ...
          <div style={{ fontSize: 11, color: res.chg >= 0 ? "#00d97e" : "#ff4560", fontWeight: 600 }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 4, minWidth: 120 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
            <span style={{ fontSize: 9, color: "#2a5060" }}>VPA</span>
            <VPABar score={res.vpa} />
          </div>
          <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
            <span style={{ fontSize: 9, color: "#2a5060" }}>RSI</span>
            <span style={{ fontSize: 11, color: res.rsi > 70 ? "#ff4560" : res.rsi < 30 ? "#00d97e" : "#b040e0", fontWeight: 600 }}>
              {res.rsi}
            </span>
            <span style={{ fontSize: 9, color: "#2a5060" }}>Vol</span>
            <span style={{ fontSize: 10, color: "#f0c040" }}>{res.volRatio}x</span>
          </div>
        </div>

        {/* Arrow */}
        <div style={{ fontSize: 12, color: "#2a5060", transform: open ? "rotate(180deg)" : "none", transition: "0.2s" }}>▼</div>
      </div>
      ...
      {open && (
        <div style={{
          ...
          {/* Entry & targets */}
          <div>
            <div style={{ fontSize: 9, color: "#2a5060", letterSpacing: 1, marginBottom: 8 }}>GIÁ VÀO / MỤC TIÊU</div>
            <div style={{ display: "flex", flexDirection: "column", gap: 5 }}>
              ...
              <div style={{ display: "flex", justifyContent: "space-between" }}>
                <span style={{ fontSize: 11, color: "#00d97e" }}>Target 1</span>
                <span style={{ fontSize: 12, fontWeight: 700, color: "#00d97e" }}>
              ...
              <div style={{ display: "flex", justifyContent: "space-between" }}>
                <span style={{ fontSize: 11, color: "#00d97e" }}>Target 2</span>
                <span style={{ fontSize: 12, fontWeight: 700, color: "#00d97e" }}>
              ...
              <div style={{ borderTop: "1px solid #0d1f2e", paddingTop: 5, display: "flex", justifyContent: "space-between" }}>
                <span style={{ fontSize: 11, color: "#ff4560" }}>Stop Loss</span>
                <span style={{ fontSize: 12, fontWeight: 700, color: "#ff4560" }}>
          ...
          {/* Signal reasons */}
          <div>
            <div style={{ fontSize: 9, color: "#2a5060", letterSpacing: 1, marginBottom: 8 }}>VSA / VPA SIGNALS</div>
            <div style={{ display: "flex", flexDirection: "column", gap: 5 }}>
              {res.vsaBullish.map(s => (
                <div key={s} style={{ fontSize: 11, color: "#00d97e", display: "flex", alignItems: "center", gap: 6 }}>
                  <span style={{ fontSize: 8 }}>▲</span> {s}
                </div>
              ))}
              {res.vsaBearish.map(s => (
                <div key={s} style={{ fontSize: 11, color: "#ff4560", display: "flex", alignItems: "center", gap: 6 }}>
                  <span style={{ fontSize: 8 }}>▼</span> {s}
                </div>
              ))}
              {res.reasons.map(r => (
              ...
              {res.goldenX && (
                <div style={{ fontSize: 11, color: "#f0c040", display: "flex", alignItems: "center", gap: 6 }}>
          ...
          {/* Risk/Reward + action */}
          <div>
            <div style={{ fontSize: 9, color: "#2a5060", letterSpacing: 1, marginBottom: 8 }}>ĐÁNH GIÁ</div>
            <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
              <RRBadge rr={res.riskReward} />
              <div style={{ fontSize: 10, color: "#2a5060" }}>
                Vol: <span style={{ color: "#f0c040" }}>{(res.volume / 1e6).toFixed(2)}M</span>
                {" "}({res.volRatio}× TB20)
              </div>
              <div style={{ fontSize: 10, color: res.uptrend ? "#00d97e" : "#ff4560" }}>
              ...
              <div style={{ marginTop: 4, display: "flex", gap: 6 }}>
                <button
                  ...
                  style={{
                    ...
                    background: color, color: "#03080e", border: "none",
...
function PortfolioTab({ id, meta, stocks, active, onClick }) {
  return (
    ...
    >
      ...
      <div style={{ fontSize: 11, fontWeight: 700, color: active ? meta.color : "#2a5060" }}>
        {meta.label}
      </div>
      <div style={{ fontSize: 9, color: active ? meta.color + "aa" : "#1e4050", marginTop: 1 }}>
        {meta.sub}
      </div>
      <div style={{
        marginTop: 5, fontSize: 11, fontWeight: 700,
        color: stocks > 0 ? meta.color : "#1e4050",
        background: stocks > 0 ? meta.color + "20" : "#0d1f2e",
  ...
  return (
    <div style={{
      ...
      {[
        ...
        { label: "VPA Score TB",            value: avgVPA + "/20", color: "#00d97e" },
        { label: "R/R tốt nhất",            value: topRR ? topRR.riskReward + "x (" + topRR.ticker + ")" : "—", color: "#f0c040" },
        { label: "Scanner cập nhật",        value: new Date().toLocaleTimeString("vi-VN"), color: "#3b9eff" },
      ].map(s => (
        <div key={s.label} style={{
          background: "#05101a", border: "1px solid #0d1f2e", borderRadius: 7, padding: "10px 12px",
        }}>
          <div style={{ fontSize: 9, color: "#2a5060", marginBottom: 4, letterSpacing: 0.5 }}>{s.label.toUpperCase()}</div>
  ...
  return (
    <div className="fade-in">
      ...
      <div style={{ marginBottom: 16, display: "flex", alignItems: "flex-start", justifyContent: "space-between", flexWrap: "wrap", gap: 10 }}>
        <div>
          <div style={{ fontSize: 11, color: "#2a5060", letterSpacing: 1, marginBottom: 4 }}>
            VSA/VPA FUND MANAGER SCANNER
          </div>
          <h2 style={{ margin: 0, fontSize: 18, fontWeight: 700, color: "#fff" }}>
            4 Danh Mục Đầu Tư
          </h2>
          <div style={{ fontSize: 11, color: "#2a5060", marginTop: 3 }}>
        ...
        <button
          ...
          style={{
            ...
            fontFamily: "inherit",
            background: isRunning ? "#1a3a50" : "#00d97e",
            color: isRunning ? "#3a7a90" : "#03080e",
      ...
      {!ran && !isRunning && (
        <div style={{
          background: "#05101a", border: "1px dashed #1a3a50",
          ...
          <div style={{ fontSize: 11, color: "#1e4050" }}>
      ...
      {/* Running */}
      {isRunning && (
        <div style={{ textAlign: "center", padding: "32px 0", color: "#3b9eff", fontSize: 12 }}>
      ...
      {ran && !isRunning && (
        <>
          ...
          {/* Portfolio tabs */}
          <div style={{
            display: "flex", background: "#05101a",
            border: "1px solid #0d1f2e", borderRadius: 8,
          ...
          <div style={{
            ...
            <div>
              ...
              <div style={{ fontSize: 11, color: "#2a5060", marginTop: 2 }}>{meta.desc}</div>
            </div>
          ...
          {activeStocks.length === 0 ? (
            <div style={{
              background: "#05101a", border: "1px dashed #0d1f2e",
              borderRadius: 8, padding: "28px 16px", textAlign: "center",
              color: "#1e4050", fontSize: 12,
            }}>
          ...
          <div style={{
            ...
            borderRadius: 6, fontSize: 10, color: "#1e4050", lineHeight: 1.7,
          }}>
